"""Train inverted-Mel CNN on the full readable ASVspoof 2019 PA train set.

Defaults use all readable files (no --max-train / --max-val caps).
Validation is speaker-disjoint within PA train by default, or PA dev
when --validate-on-dev is set (closer to official protocol).

Does not modify inverted_mel_pa2019_train/ (kept for subset / smoke runs).
"""
...
def save_artifacts(
    ...
    axis.set(
        ...
        title="ROC curve (full PA2019 train)",
    )
    axis.legend()
    fig.tight_layout()
...
def _build_train_val_records(
    pa_root: Path,
    cache_dir: Path,
    args: argparse.Namespace,
) -> tuple[list, list, str, list[str]]:
    """Return (train_records, val_records, val_split_name, skipped_utts)."""
    ...
    if args.validate_on_dev:
        # Use 100% of readable PA train; validate on readable PA dev.
        ...
        dev_protocol = pa_root / PROTOCOL_BY_SPLIT["dev"]
        all_dev = read_pa_cm_protocol(dev_protocol)
        print(f"PA dev protocol: {len(all_dev)} files")
        readable_val, skipped_val = filter_readable_records(
            pa_root,
            "dev",
            all_dev,
            cache_path=cache_dir / "pa2019_dev_readable.json",
            force_refresh=args.refresh_cache,
            target_count=0,
        )
        skipped = sorted(set(skipped_train) | set(skipped_val))
        return readable_train, readable_val, "pa2019_dev", skipped

    # Speaker-disjoint holdout within PA train (full readable pools).
    ...
    skipped = sorted(set(skipped_train) | set(skipped_val))
    return readable_train, readable_val, "pa2019_train_speaker_val", skipped
...
def train(args: argparse.Namespace) -> None:
    ...
    print(
        f"FULL PA2019 train | feature={config.feature_type} | device={device} | "
        f"validate_on_dev={args.validate_on_dev}"
    )

    train_records, val_records, val_split_name, skipped = _build_train_val_records(
        pa_root, cache_dir, args
    )
    if skipped:
    ...
    print(
        f"Val ({val_split_name}): {len(val_records)} files / {n_val_spk} speakers "
        f"(bona={sum(r.label == 0 for r in val_records)}, "
        f"spoof={sum(r.label == 1 for r in val_records)})"
    )
    if not train_records or not val_records:
        raise ValueError("Empty train or val set after readability filtering")

    val_audio_split = "dev" if args.validate_on_dev else "train"
    ...
    ckpt_path = output_dir / "best_inverted_mel_pa2019_full.pt"
    history: list[dict] = []
            ...
            torch.save(
                {
                    ...
                    "experiment": "inverted_mel_pa2019_full",
                    "validate_on_dev": args.validate_on_dev,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=_THIS_DIR / "runs" / "inverted_mel_pa2019_full",
    )
    parser.add_argument(
        "--cache-dir",
        type=Path,
        default=_THIS_DIR / "cache",
        help="Readable-utt caches (can reuse sibling experiment caches via symlink)",
    )
    parser.add_argument(
        "--feature-type",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="Speaker holdout fraction when not using --validate-on-dev",
    )
    parser.add_argument(
        "--validate-on-dev",
        action="store_true",
        help="Train on all readable PA train; early-stop / threshold on PA dev",
    )

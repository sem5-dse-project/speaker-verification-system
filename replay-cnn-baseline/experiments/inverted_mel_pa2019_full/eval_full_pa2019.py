"""Evaluate a full-PA2019 inverted-Mel checkpoint on ASVspoof 2019 PA splits.

Scores all readable files on the chosen split (dev / eval / train).
Reports metrics at the checkpoint threshold and at an oracle EER threshold.
"""
...
_DEFAULT_CKPT = (
    _THIS_DIR / "runs" / "inverted_mel_pa2019_full" / "best_inverted_mel_pa2019_full.pt"
)
...
def collect_scores(model, loader, device):
    ...
    for waveforms, batch_labels, batch_ids in tqdm(
        loader, desc="Scoring full PA split"
    ):
...
def run_eval(args: argparse.Namespace) -> None:
    ...
    print(
        ...
        f"num_train={ckpt.get('num_train')}"
    )

    protocol = pa_root / PROTOCOL_BY_SPLIT[args.split]
    records = read_pa_cm_protocol(protocol)
    print(f"PA {args.split} protocol: {len(records)} files")
    random.seed(args.seed)
    random.shuffle(records)

    # Full readable set only (no max_files subset).
    ...
    print(
        f"PA {args.split}: readable={len(records)}, skipped_corrupt={len(skipped)}"
    )
    if skipped:
    ...
    if not records:
        raise ValueError(f"No readable files for PA {args.split}")
    ...
    result = {
        ...
        "num_protocol_files": int(len(read_pa_cm_protocol(protocol))),
    ...
    prefix = f"pa2019_{args.split}_full"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=_THIS_DIR / "runs" / "eval_pa2019_dev_full",
    )
